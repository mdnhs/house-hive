...
// ---------------------------------------------------------------------------
// MapLibre GL Complex Text Plugin — renders Bengali, Devanagari, etc.
// Uses the RTL plugin hook with HarfBuzz for proper glyph shaping.
// ---------------------------------------------------------------------------
const COMPLEX_TEXT_PLUGIN_URL =
  "https://wipfli.github.io/maplibre-gl-complex-text/dist/maplibre-gl-complex-text.js";

/** Unicode range starts that the plugin maps to pre-positioned glyphs in the PGF font. */
const PUA_RANGE_STARTS = new Set([
  63488, 63232, 62976, 62720, 62464, 62208, 61952, 61696,
  61440, 61184, 60928, 60672, 60416, 60160, 59904, 59648,
  59392, 59136, 58880, 58624, 58368, 58112, 57856, 57600,
  3072, 2816, 2560, 2304, 10240, 10752,
]);

const PGF_GLYPH_SERVER =
  "https://wipfli.github.io/pgf-glyph-ranges/font/NotoSansMultiscript-Regular-v1";

let rtlPluginRegistered = false;

/**
 * Register the MapLibre GL Complex Text plugin exactly once.
 * Must be called client-side before any Map instance renders text in
 * Bengali, Devanagari, or other CTL scripts.
 */
function ensureComplexTextPlugin() {
  if (rtlPluginRegistered) return;
  rtlPluginRegistered = true;
  // setRTLTextPlugin loads & evaluates the plugin script. The plugin hooks
  // into MapLibre's text pipeline and uses HarfBuzz (WASM) to shape CTL
  // scripts, encoding resulting glyphs into Private Use Area codepoints.
  MapLibreGL.setRTLTextPlugin(COMPLEX_TEXT_PLUGIN_URL, false);
}

/**
 * Transform-request interceptor: redirects PUA-range glyph requests to the
 * pre-built positioned-glyph font (PGF) server, so MapLibre can find the
 * shaped glyphs produced by the complex text plugin.
 */
function complexTextTransformRequest(
  url: string,
  ...
  if (!PUA_RANGE_STARTS.has(start)) return undefined;
  return { url: `${PGF_GLYPH_SERVER}/${match[1]}-${match[2]}.pbf` };
}
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    // Register the complex text plugin for Bengali/complex script support.
    // Must happen before or during map creation; MapLibre queues text
    // processing until the plugin is ready.
    try {
      ensureComplexTextPlugin();
    } catch {
      // Plugin already registered or failed to load — proceed without it.
    }
    ...
    const map = new MapLibreGL.Map({
      ...
      transformRequest: complexTextTransformRequest,
      ...props,
      ...viewport,
    });
...
type MapMarkerProps = {
  ...
  children: ReactNode;
  /** When hovered, raises the marker's z-index above others */
  hovered?: boolean;
...
function MapMarker({
  ...
  // Raise the marker's root element when hovered so it stacks above others
  useEffect(() => {
